...
We used Gradient-weighted Class Activation Mapping (Grad-CAM) to inspect which image regions support the phase model's class score \cite{selvaraju2017gradcam}. Grad-CAM weights convolutional feature maps by the gradient of a selected class score and returns a coarse, class-specific localization map. For our spatiotemporal model, the resulting maps can be overlaid on frames sampled throughout a five-second clip. Red and yellow denote stronger positive support for the displayed phase score; blue denotes weaker support. These maps explain spatial sensitivity, not the complete temporal decision rule.
...
\begin{center}
  ...
  \captionof{figure}{Phase-conditioned Grad-CAM examples. Each column shows an input frame (top) and its heatmap overlay (bottom). The displayed confidence is the model's score for the predicted phase; it should not be interpreted as calibrated reliability.}
  \label{fig:gradcam-phase}
\end{center}

\begin{center}
  \includegraphics[width=0.73\textwidth]{figures/gradcam_temporal.png}
  \captionof{figure}{Temporal Grad-CAM overlays at eight sampled times across representative five-second clips. The changing hotspot provides a qualitative view of where spatial evidence is located over time.}
