...
\begin{center}
  ...
  \vspace{0.4mm}
  {\small\href{https://github.com/Maximilianb1/mma-fight-analyzer}{github.com/Maximilianb1/mma-fight-analyzer}\par}
\end{center}
\vspace{1mm}
...
\section{Dataset and Task Definitions}

We used a custom Streamlit labeler to divide 11 UFC fights into consecutive, non-overlapping five-second clips. The typical source is 1280$\times$720 at 29.97 fps. For each live-action clip, the annotator selected one phase and one pressure label.

\textbf{Fight versus non-fight.} Here, \emph{fight} does not mean that the system decides whether arbitrary people are behaving violently. It specifically means that the five-second window shows the live UFC bout in progress during an active round. Circling, measuring distance, clinching, and ground control still count as fight footage even when no strike is thrown. \emph{Non-fight} means surrounding broadcast content rather than live action: for example a replay, walkout, fighter introduction, between-round break, crowd shot, interview, or studio/graphic segment. These windows were retained as \emph{excluded} clips to train the gate. At inference, the gate labels them non-fight and prevents the phase and pressure models from assigning an MMA interpretation to irrelevant footage.

\textbf{Phase.} Phase describes the main physical mode of the action, not who is winning. \emph{Striking} is an open-distance standing exchange dominated by punches or kicks. \emph{Grappling/Ground Work} covers action on the canvas, such as positional control, submissions, escapes, or ground strikes. \emph{Clinch} is close upright body contact---often against the cage---where fighters hold, hand-fight, or strike at short range. \emph{Transition/Takedown} is an active change between stable positions, including a takedown attempt, throw, scramble, stand-up, or movement from standing to ground. \emph{Neutral/Measuring Distance} means both fighters are standing apart, circling, feinting, or probing without a committed exchange. A clip can cross a boundary, but the interface requires the annotator to choose the single phase that best represents the five-second window.

\textbf{Pressure.} Pressure is a directional description of initiative: which fighter is making the opponent react, typically through forward movement, sustained attack, cage control, or positional control. It is not the official score, damage dealt, number of strikes landed, or a prediction of the winner. The label is \emph{Fighter 1} when that fighter is judged to be imposing the action, \emph{Fighter 2} for the reverse, and \emph{Mutual} when the initiative is balanced or neither fighter clearly controls it. Fighter 1 is defined consistently as the name shown left of the broadcast timer; Fighter 2 is the name on the right.

Figure~\ref{fig:data} shows the pronounced imbalance: Striking represents 652/1,159 live clips, whereas Clinch has only 57. Pressure is also imbalanced toward Mutual (567 clips).
...
\begin{figure*}[t]
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \caption{Correctly tracked Striking window: Paddy advances with a jab while Chandler defends.}
  \end{subfigure}\hfill
  \begin{subfigure}{0.49\textwidth}
...
We used EigenGradCAM, as implemented by Gildenblat et al. \cite{gildenblat2021cam}, to inspect which image regions support the phase model's class score. This Grad-CAM variant projects activation--gradient products onto their first principal component, producing a class-specific localization volume \cite{selvaraju2017gradcam}. We target the final blocks of backbone layers 3 and 4 and average their resized maps. Red and yellow denote stronger positive support for the displayed phase score; blue denotes weaker support. The supplied script searched up to 40 labeled clips per phase and retained the highest-confidence correct prediction; its candidate pool was not restricted to unseen clips. These selected examples are therefore diagnostics, not evaluation results.

Figure~\ref{fig:gradcam-phase} shows one high-confidence example for each predicted phase. Activation generally overlaps the fighters or their interaction region: Grappling/Ground Work and Clinch concentrate on coupled upper bodies, while Transition/Takedown emphasizes grounded contact. However, Striking and Neutral/Measuring Distance also activate around the lower frame, including the timer and mat graphics. This suggests a possible broadcast-context shortcut in addition to fighter motion, although the maps do not quantify its effect. Figure~\ref{fig:gradcam-time} follows representative clips across eight sampled times. Hotspots move as the fighters change position and often remain centered on their interaction, while sometimes spreading over the cage, canvas, and overlay.

We therefore treat Grad-CAM as a diagnostic, not evidence of human-like MMA reasoning. The examples are selected, there is no ground-truth saliency, the maps are coarse and non-causal, and they do not explain the separate pressure model. Follow-up work should evaluate the complete holdout and measure prediction changes when fighter, timer, and background regions are independently masked.

\begin{center}
  \includegraphics[width=0.80\textwidth]{figures/gradcam_by_phase.png}
  \captionof{figure}{Phase-conditioned EigenGradCAM examples. Each column shows an input frame (top) and its heatmap overlay (bottom). The displayed confidence is the model's score for the correctly predicted phase; it should not be interpreted as calibrated reliability.}
  \label{fig:gradcam-phase}
\end{center}

\begin{center}
  \includegraphics[width=0.73\textwidth]{figures/gradcam_temporal.png}
  \captionof{figure}{Temporal EigenGradCAM overlays at eight sampled times across representative five-second clips. Rows follow the phase order in Figure~\ref{fig:gradcam-phase}. The changing hotspot shows where spatial evidence is located over time.}
  \label{fig:gradcam-time}
\end{center}
...
\newpage
\subsection{Project scope and proof of concept}

This project deliberately pursued a broader scope than a single clip classifier. We created and labeled a task-specific dataset from scratch and connected broadcast-content filtering, spatiotemporal classification, fighter detection, identity assignment, temporal tracking, and audiovisual rendering into one runnable system. We did not find an existing dataset that jointly provides our five phase labels, directional pressure, non-fight examples, and fighter identity for UFC broadcasts. We therefore view the present system as an end-to-end proof of concept rather than a finished product. Its imperfect pressure results are important evidence of the remaining difficulty, while the functioning demo establishes that the complete pipeline is technically feasible. We believe that substantially more annotated fights, independent labels, stronger identity models, broader hyperparameter exploration, and greater compute could turn this concept into a useful video-indexing and analysis tool. We invite the course staff to run the included demo and assess the complete system alongside its measured limitations.
...
\noindent\textbf{Maximilian Bershtman:} I feel like chatgpt covered the explanations in pretty ethical way but there are few stuff that can make the explanations more ethical. For example the ai could have methoined that the data set is small and nade by one person and maybe not relaible not only to the third group of steak holders but also to the other groups. also maybe it would be good to also mention that even the phase classification that worked better then the pressure classification is also not very realible because of the limitations of this research. \textbf{Reut Yosefa Vitzner:} The generative AI responses should include its confidence levels along with explanations of their known limitations, assumptions and biases. This would help stakeholders understand when the model is uncertain and avoid treating its predictions as factual. In addition, adding explanations regarding what influenced its decision making would help users evaluate the decision and understand it's source.
