...
class R2Plus1DDual(nn.Module):
    ...
    def __init__(self, in_channels=4, with_phase=True, with_pressure=True,
                 pretrained=True, dropout=0.4, pressure_head="flat"):
        super().__init__()
...
class VideoResNetDual(nn.Module):
    ...
    def __init__(self, name, in_channels=4, with_phase=True, with_pressure=True,
                 pretrained=True, dropout=0.4, pressure_head="flat"):
        super().__init__()
...
def build_phase_model(name, in_channels=4, with_phase=True, with_pressure=True,
                      pretrained=True, pressure_head="flat"):
    if name == "r2plus1d":
        return R2Plus1DDual(in_channels, with_phase, with_pressure, pretrained,
                            pressure_head=pressure_head)
    if name in ("r3d", "mc3"):
        return VideoResNetDual(name, in_channels, with_phase, with_pressure, pretrained,
                               pressure_head=pressure_head)
    if name == "lstm":
