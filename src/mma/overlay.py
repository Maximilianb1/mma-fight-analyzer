...
def draw_fighter_box(frame, box, name, color):
    ...
    cv2.putText(frame, name, (x1 + 4, y1 - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
...
def draw_banner(frame, lines, alpha=0.65, text_color=(255, 255, 255)):
    ...
    pad, lh = 12, 30
    width = max(cv2.getTextSize(t, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0][0] for t in lines)
    h = pad * 2 + lh * len(lines)
    overlay = frame.copy()
    cv2.rectangle(overlay, (0, 0), (width + pad * 3, h), BANNER_BG, -1)
    cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0, frame)
    for i, text in enumerate(lines):
        cv2.putText(frame, text, (pad, pad + lh * i + 20), cv2.FONT_HERSHEY_SIMPLEX,
                    0.7, text_color, 2, cv2.LINE_AA)


def annotate_window(frames, sampled_idx, f1_boxes, f2_boxes, phase, pressure,
                    f1_name="Fighter 1", f2_name="Fighter 2", excluded=False,
                    phase_conf=None, pressure_conf=None):
    """Draw boxes + labels on every frame of a 5s window, in place.
    Low-confidence windows get a dimmed gray banner."""
    if excluded:
        for frame in frames:
            draw_banner(frame, ["NON-FIGHT SEGMENT (replay / break)"])
        return
    ...
    for f, frame in enumerate(frames):
        ...
        if lines:
            draw_banner(frame, lines, alpha=0.45 if dim else 0.65,
...
def save_identity_prompt_image(path, frame, box_a, box_b):
    ...
    for box, tag, color in ((box_a, "A", (0, 255, 0)), (box_b, "B", (0, 255, 255))):
        ...
        cv2.putText(img, tag, (x1 + 8, y1 + 45), cv2.FONT_HERSHEY_SIMPLEX, 1.6, color, 4)
