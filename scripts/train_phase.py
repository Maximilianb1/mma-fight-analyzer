...
from mma.train_utils import (class_weights, evaluate, make_holdout_folds,  # noqa: E402
...
def aggregate(out_dir, model_name, k, holdout_fight, fold_pairs):
    ...
    for task, labels in (("phase", C.PHASE_LABELS), ("pressure", C.PRESSURE_LABELS)):
        ...
        fold_f1 = [float(f1_score(p[f"{task}_true"], p[f"{task}_pred"],
                                  labels=range(len(labels)), average="macro", zero_division=0))
                   for p in parts if len(p[f"{task}_true"])]
    ...
    for fight in sorted(np.unique(fights)):
        ...
        if has_phase:
            ...
            row["phase_macro_f1"] = float(f1_score(
                ph_true[m], ph_pred[m], labels=range(C.NUM_PHASE_CLASSES),
...
def save_holdout_results(out_dir, tag, holdout_fight, records, evaluation):
    ...
    result = {"protocol": "one-shot untouched fight test",
              "holdout_fight": holdout_fight, "n_clips": int(len(records)),
              "tasks": {}}
    np.savez(out_dir / f"{tag}_holdout_preds.npz",
             phase_true=evaluation["phase_true"], phase_pred=evaluation["phase_pred"],
    ...
    print(f"Untouched holdout results saved to {out_dir / f'{tag}_holdout_metrics.json'}")
...
def main():
    ...
    p.add_argument("--k", type=int, default=C.DEV_FOLDS,
                   help="development folds; default gives two validation fights per fold")
    p.add_argument("--holdout-fight", default=C.DEFAULT_HOLDOUT_FIGHT,
    ...
    dev, test, paired_folds, fold_pairs = make_holdout_folds(
        records, split_records=all_records, holdout_fight=args.holdout_fight,
        n_splits=args.k)
    ...
    if args.final:
        ...
        loader = make_loader(dev, args.cache_dir, True, mean, std,
        ...
        train_model(model, loader, None, device, ckpt, meta,
                    epochs=final_epochs, lr=args.lr,
        ...
        holdout_eval = evaluate(
            model, make_loader(test, args.cache_dir, False, mean, std,
                               not args.no_mask, batch, args.workers),
    ...
    if args.lofo:
        fold_pairs = [tuple(sorted(dev.iloc[va_idx].fight.unique())) for _, va_idx in folds]
    k = len(folds)
