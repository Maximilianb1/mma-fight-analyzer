...
def make_loader(records, cache_dir, train, batch_size, workers):
    return DataLoader(
        GateClipDataset(records, cache_dir, train),
...
def collect(model, loader, device):
    ...
    return (
        np.concatenate(frame_probs),
        np.concatenate(clip_probs),
...
def choose_thresholds(labels, probs, min_fight_retention):
    ...
    max_f1_threshold = (
        float(thresholds[int(np.argmax(f1[:-1]))]) if len(thresholds) else 0.5
    )
...
def save_plots(out_dir, stem, histories, labels, probs, threshold, title_suffix):
    ...
    for fold, history in enumerate(histories):
        axes[0].plot(
            history["epoch"], history["train_loss"], marker="o", label=f"fold {fold}"
        )
    ...
    axes[1].set(
        title=f"Clip-level ROC ({title_suffix})",
    ...
    axes[2].set(
        title=f"Clip-level precision-recall ({title_suffix})",
        xlabel="Recall",
        ylabel="Precision",
    )
    axes[2].legend()
    ...
    sns.heatmap(
        ...
        fmt="d",
        cmap="Blues",
        xticklabels=["Fight", "Non-fight"],
        yticklabels=["Fight", "Non-fight"],
        ax=ax,
    )
    ax.set(
        xlabel="Predicted",
...
def train_fold(fi, train_records, val_records, pair, args, device, out_dir):
    ...
    train_loader = make_loader(
        train_records, args.cache_dir, True, args.batch_size, args.workers
    )
    val_loader = make_loader(
        val_records, args.cache_dir, False, args.batch_size, args.workers
    )
    model = GateNet().to(device)
    positives = int(train_records.excluded.sum())
    pos_weight = torch.tensor(
        (len(train_records) - positives) / max(positives, 1), device=device
    )
    ...
    print(
        f"fold {fi + 1}/{args.k}: train {len(train_records)} clips; "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"[gate f{fi}] epoch {epoch}/{args.epochs} loss={train_loss:.4f} "
            ...
            torch.save(
                {
                    ...
                    "meta": {
                        "fold": fi,
                        "best_epoch": epoch,
                        "val_auc": float(clip_auc),
                        "val_ap": float(clip_ap),
                        "frame_val_auc": float(frame_auc),
                        "val_fights": list(pair),
                    },
                },
                checkpoint,
            )
    ...
    frame_probs, clip_probs, labels = collect(model, val_loader, device)
    np.savez(
        out_dir / f"gate_fold{fi}_preds.npz",
        frame_probability=frame_probs,
        probability=clip_probs,
        target=labels,
...
def aggregate_folds(out_dir, pairs, args):
    ...
    for fi, (part, pair) in enumerate(zip(parts, pairs)):
        ...
        fold_result.update(
            {"fold": fi, "val_fights": list(pair), "n_clips": int(len(part["target"]))}
        )
        fold_metrics.append(fold_result)
        histories.append(
            json.loads((out_dir / f"gate_fold{fi}_history.json").read_text())
        )
        state = torch.load(
            out_dir / f"gate_fold{fi}.pt", map_location="cpu", weights_only=False
        )
    ...
    np.savez(
        out_dir / "gate_oof_predictions.npz",
        probability=probs,
        target=labels,
    ...
    print(
        f"OOF AUC={oof['auc']:.3f} AP={oof['ap']:.3f} F1={oof['f1']:.3f} "
...
def train_final(dev, test, args, device, out_dir):
    ...
    if not metrics_path.exists():
        raise SystemExit(
            "run all development folds and --aggregate-only before --final"
        )
    cv = json.loads(metrics_path.read_text(encoding="utf-8"))
    if cv.get("holdout_fight") != args.holdout_fight or "oof" not in cv:
        raise SystemExit(
            "gate_metrics.json is not from the requested holdout-CV protocol"
        )
    ...
    test_loader = make_loader(
        test, args.cache_dir, False, args.batch_size, args.workers
    )
    ...
    holdout.update(
        {
            ...
            "holdout_fight": args.holdout_fight,
            "n_clips": int(len(test)),
            "threshold_source": "development OOF",
        }
    )
    checkpoint_meta = {
        "threshold": threshold,
    ...
    torch.save(
        {"state_dict": model.state_dict(), "meta": checkpoint_meta}, out_dir / "gate.pt"
    )
    (out_dir / "gate_final_history.json").write_text(
        json.dumps(history, indent=2), encoding="utf-8"
    )
    (out_dir / "gate_holdout_metrics.json").write_text(
        json.dumps(holdout, indent=2), encoding="utf-8"
    )
    np.savez(
        out_dir / "gate_holdout_predictions.npz",
        frame_probability=frame_probs,
        probability=probs,
        target=labels,
        prediction=(probs >= threshold).astype(int),
        fight=test.fight.values.astype(str),
        filename=test.filename.values.astype(str),
    )
    save_plots(
        out_dir, "gate_holdout", [], labels, probs, threshold, "untouched holdout"
    )
    print(
        f"HOLDOUT {args.holdout_fight}: AUC={holdout['auc']:.3f} "
...
def main():
    ...
    parser.add_argument(
        "--folds", default="all", help="'all' or comma list, e.g. 0,2,4"
    )
    ...
    for fi in wanted:
        ...
        train_fold(
            fi, dev.iloc[train_idx], dev.iloc[val_idx], pairs[fi], args, device, out_dir
        )
    if args.folds == "all":
        aggregate_folds(out_dir, pairs, args)
    else:
        print(
            "selected folds complete; run --aggregate-only after all five are present"
        )
