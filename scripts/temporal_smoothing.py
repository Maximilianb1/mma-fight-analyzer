...
def main():
    ...
    p.add_argument(
        "--method",
        choices=["all", "probability_mean", "majority_vote"],
        default="all",
    ...
    files = (
        sorted(out_dir.glob(f"{args.tag}_fold*_preds.npz"))
        if args.source == "oof"
    ...
    for task in tasks:
        ...
        if f"{task}_prob" not in parts[0]:
            raise SystemExit(
                "prediction files predate probability saving; rerun evaluation/training"
            )
        probs = np.concatenate([x[f"{task}_prob"] for x in parts])
        raw = probs.argmax(axis=1)
        mean_pred = smooth_probabilities(probs, fights, filenames, args.window).argmax(
            axis=1
        )
        majority_pred = majority_smoothing(
            raw, fights, filenames, args.window, len(labels)
        )
        candidates = {
        ...
        selected = (
            candidates
            if args.method == "all"
            else {args.method: candidates[args.method]}
        )
        results.update(
            {name: metrics(y, pred, labels) for name, pred in selected.items()}
        )
        all_results["tasks"][task] = results
        print(f"\n{task.upper()} / {args.tag}")
        for name, result in results.items():
            print(
                f"  {name:<18} acc={result['accuracy']:.3f} macro-F1={result['macro_f1']:.3f}"
            )
        plot_comparison(
            out_dir / f"smoothing_{args.source}_{task}_{args.tag}.png",
            task,
            labels,
            results,
            args.source,
